...
import uk.ac.ebi.eva.lib.repositories.DgvaStudyBrowserRepositoryOld;
...
@Component
@Deprecated
public class StudyDgvaDBAdaptorOld implements StudyDBAdaptor {

    @Autowired
    private DgvaStudyBrowserRepositoryOld dgvaStudyBrowserRepository;
    ...
    public QueryResult getAllStudies(QueryOptions queryOptions) {
        ...
        List<DgvaStudyBrowserOld> dgvaStudies = dgvaStudyBrowserRepository.findAll(filterSpecification);
        List<VariantStudy> variantstudies = new ArrayList<>();
        for (DgvaStudyBrowserOld dgvaStudy : dgvaStudies) {
    ...
    public QueryResult getStudyById(String studyId, QueryOptions queryOptions) {
        long start = System.currentTimeMillis();
        DgvaStudyBrowserOld dgvaStudy = dgvaStudyBrowserRepository.getOne(studyId);
