...
@DataJpaTest
@Deprecated
public class ArchiveDgvaDBAdaptorOldTest {
    ...
    public void setUp() throws Exception {
        DgvaStudyOldTestData.persistTestData(entityManager);
    }
    ...
    public void countStudiesPerSpeciesFilteringBySpeciesAndType() throws Exception {
        ...
        queryOptions.put(QueryOptionsConstants.TYPE, DgvaStudyOldTestData.CONTROL_SET);
    ...
    public void countStudiesPerTypeFilteringByType() throws Exception {
        QueryResult<Map.Entry<String, Long>> queryResult = archiveDgvaDBAdaptor
                .countStudiesPerType(new QueryOptions(QueryOptionsConstants.TYPE, DgvaStudyOldTestData.CONTROL_SET));

        assertEquals(1, queryResult.getNumTotalResults());
        Map.Entry<String, Long> result = queryResult.first();
        assertEquals(DgvaStudyOldTestData.CONTROL_SET, result.getKey());
        assertEquals(2, result.getValue().longValue());
    }
    ...
    public void countStudiesPerTypeUnfiltered() throws Exception {
        ...
        assertEquals(2, queryResult.getNumTotalResults());
        List<Map.Entry<String, Long>> results =  queryResult.getResult();
        long controlSetStudiesCount = results.stream().filter(e -> e.getKey().equals(DgvaStudyOldTestData.CONTROL_SET))
                                        .mapToLong(Map.Entry::getValue).findAny().getAsLong();
        long collectionStudiesCount = results.stream().filter(e -> e.getKey().equals(DgvaStudyOldTestData.COLLECTION))
                                        .mapToLong(Map.Entry::getValue).findAny().getAsLong();
